...
class TestAbuse(TestCase):
    ...
    def test_search_user(self):
        ...
        response = self.client.get(
            self.list_url,
            {'q': 'NotGoingToFindAnything', 'type': 'user'},
            follow=True,
        )
        assert response.status_code == 200
        doc = pq(response.content)
    ...
    def test_search_addon(self):
        ...
        response = self.client.get(
            self.list_url,
            {'q': str(self.addon1.pk), 'type': 'addon'},
            follow=True,
        )
        assert response.status_code == 200
        doc = pq(response.content)
        ...
        response = self.client.get(
            self.list_url,
            {'q': 'NotGoingToFindAnything', 'type': 'addon'},
            follow=True,
        )
        assert response.status_code == 200
        doc = pq(response.content)
    ...
    def test_filter_by_state(self):
        response = self.client.get(
            self.list_url,
            {'state__exact': AbuseReport.STATES.VALID},
            follow=True,
        )
        assert response.status_code == 200
        doc = pq(response.content)
    ...
    def test_filter_by_reason(self):
        response = self.client.get(
            self.list_url,
            {'reason__exact': AbuseReport.REASONS.OTHER},
            follow=True,
        )
        assert response.status_code == 200
        doc = pq(response.content)
    ...
    def test_action_mark_multiple_as_valid(self):
        ...
        action_callback = abuse_report_admin.get_actions(request)[
            'mark_as_valid'
        ][0]

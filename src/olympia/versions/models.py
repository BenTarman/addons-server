...
class Version(OnChangeMixin, ModelBase):
    ...
    @classmethod
    def from_upload(
        cls, upload, addon, selected_apps, channel, parsed_data=None
    ):
        """
        ...
        if parsed_data.get('is_mozilla_signed_extension'):
            approval_notes = (
                u'This version has been signed with '
    ...
    def is_public(self):
        ...
        try:
            return (
                ...
                and all(
                    f.status == amo.STATUS_APPROVED for f in self.all_files
                )
            )
    ...
    def is_unreviewed(self):
        return bool(
            list(
                filter(
                    lambda f: f.status in amo.UNREVIEWED_FILE_STATUSES,
                    self.all_files,
                )
            )
        )

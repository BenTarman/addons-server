...
class TestWithUser(TestCase):
    ...
    def test_addon_developer_should_redirect_for_two_factor_auth(self):
        ...
        assert query == {
            ...
            'scope': ['profile'],
            'state': [
                'some-blob:{next_path}'.format(next_path=force_text(next_path))
            ],
        }
...
class TestAuthenticateView(TestCase, PatchMixin, InitializeSessionMixin):
                ...
                self.url,
                {
                    ...
                    'state': ':'.join(
                        [
                            self.fxa_state,
                            force_text(
                                base64.urlsafe_b64encode(b'https://go.go/here')
                            ),
                        ]
                    ),
                },
    ...
    def test_log_in_redirects_to_absolute_url(self):
                ...
                self.url,
                {
                    ...
                    'state': ':'.join(
                        [
                            self.fxa_state,
                            force_text(
                                base64.urlsafe_b64encode(next_path.encode())
                            ),
                        ]
                    ),
                },
        ...
        self.assertRedirects(
            response, next_path, fetch_redirect_response=False
        )
...
class TestAccountNotificationUnsubscribe(TestCase):
    ...
    def test_unsubscribe_user(self):
        ...
        assert UserNotification.objects.filter(
            user=self.user,
            notification_id=notification_const.id,
            enabled=False,
        ).exists()

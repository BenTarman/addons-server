...
class TestCollectionAdmin(TestCase):
    ...
    def test_can_do_limited_editing_with_admin_curation_permission(self):
        ...
        assert (
            CollectionAddon.objects.filter(collection=collection).count() == 1
        )
        ...
        post_data['collectionaddon_set-1-id'] = (
            CollectionAddon.objects.filter(collection=collection, addon=addon)
            .get()
            .pk
        )
        response = self.client.post(self.detail_url, post_data, follow=True)
        ...
        assert (
            CollectionAddon.objects.filter(
                collection=collection, addon=addon
            ).count()
            == 1
        )
        assert (
            CollectionAddon.objects.filter(
                collection=collection, addon=addon2
            ).count()
            == 0
        )

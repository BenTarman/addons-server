...
class CannedResponse(ModelBase):
    ...
    type = models.PositiveIntegerField(
        choices=amo.CANNED_RESPONSE_TYPE_CHOICES.items(),
        db_index=True,
        default=0,
    )
...
class ViewExtensionQueue(
    ExtensionQueueMixin, CombinedReviewQueueMixin, ViewQueue
):
    pass
...
class ViewThemeFullReviewQueue(
    ThemeQueueMixin, FullReviewQueueMixin, ViewQueue
):
    pass
...
class ReviewerScore(ModelBase):
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=('created',), name='reviewer_scores_created_idx'
            ),
            models.Index(fields=('user',), name='reviewer_scores_user_id_idx'),
            models.Index(
                fields=('version',), name='reviewer_scores_version_id'
            ),
        ]
...
class AutoApprovalSummary(ModelBase):
    ...
    def check_is_locked(cls, version):
        ...
        return (
            not is_langpack
            and bool(locked_by)
            and locked_by != settings.TASK_USER_ID
        )

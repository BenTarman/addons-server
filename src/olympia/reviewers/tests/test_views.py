...
class TestDashboard(TestCase):
    ...
    def test_admin_all_permissions(self):
        ...
        addon2 = addon_factory(
            name=u'Public Addön 2', status=amo.STATUS_APPROVED
        )
        AddonReviewerFlags.objects.create(
        ...
        assert (
            doc('.dashboard a')[16].text == 'Ratings Awaiting Moderation (1)'
        )
        # admin tools
        assert (
            doc('.dashboard a')[22].text == 'Expired Information Requests (2)'
        )
...
class TestModeratedQueue(QueueTest):
    ...
    def test_queue_layout(self):
        ...
        self._test_queue_layout(
            'Rating Reviews', tab_position=1, total_addons=2, total_queues=2
        )
...
class TestReview(ReviewBase):
    ...
    def test_no_versions(self):
        ...
        assert response.status_code == 302
        self.assert3xx(
            response, reverse('reviewers.queue_extension'), status_code=302
        )
...
class TestReviewPending(ReviewBase):
    ...
    def test_review_unreviewed_files(self, mock_sign):
        ...
        reviewed = File.objects.create(
            version=self.version, status=amo.STATUS_APPROVED
        )
        disabled = File.objects.create(
...
class TestPolicyView(ReviewerTest):
    ...
    def test_eula(self):
        ...
        assert response.status_code == 200
        self.assertContains(
            response, '{addon} :: EULA'.format(addon=self.addon.name)
        )

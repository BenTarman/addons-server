...
class Addon(OnChangeMixin, ModelBase):
    ...
    def force_enable(self):
        ...
        log.info(
            'Addon "%s" status changed to: %s', self.slug, amo.STATUS_APPROVED
        )
    ...
    def _prepare_deletion_email(self, msg, reason):
        ...
        context = {
            ...
            'user_str': (
                "%s, %s (%s)" % (user.name, user.email, user.id)
                if user
                else "Unknown"
            ),
        }
    ...
    def delete(self, msg='', reason='', send_delete_email=True):
        ...
        if soft_deletion:
            ...
            activity.log_create(
                amo.LOG.DELETE_ADDON, self.pk, str(self.guid), self
            )
            self.update(
...
@receiver(
    translation_saved, sender=Addon, dispatch_uid='watch_addon_name_changes'
)
...
class MigratedLWT(OnChangeMixin, ModelBase):
    ...
    static_theme = models.ForeignKey(
        Addon,
        unique=True,
        related_name='migrated_from_lwt',

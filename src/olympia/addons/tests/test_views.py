...
class TestAddonSearchView(ESTestCase):
    ...
    def test_basic(self):
        addon = addon_factory(
            slug='my-addon', name=u'My Addôn', popularity=666
        )
    ...
    def test_pagination(self):
        ...
        addon_factory(
            slug='my-third-addon', name=u'My third Addôn', popularity=11
        )
        self.refresh()
    ...
    def test_filtering_only_reviewed_addons(self):
        public_addon = addon_factory(
            slug='my-addon', name=u'My Addôn', popularity=222
        )
        addon_factory(
    ...
    def test_filter_by_appversion(self):
        ...
        data = self.perform_search(
            self.url, {'app': 'android', 'appversion': '43.0.1'}
        )
        assert data['count'] == 2
        assert len(data['results']) == 2
        assert data['results'][0]['id'] == an_addon.pk
        ...
        data = self.perform_search(
            self.url, {'app': 'android', 'appversion': '42.0.1'}
        )
        assert data['count'] == 1
        assert len(data['results']) == 1
        assert data['results'][0]['id'] == an_addon.pk
    ...
    def test_exclude_addons(self):
        ...
        data = self.perform_search(
            self.url,
            {'exclude_addons': u','.join(map(str, (addon2.pk, addon1.pk)))},
        )
        ...
        data = self.perform_search(
            self.url,
            {'exclude_addons': u','.join((str(addon1.pk), addon3.slug))},
        )
...
class TestAddonAutoCompleteSearchView(ESTestCase):
    ...
    def test_get_queryset_excludes(self):
        addon_factory(slug='my-addon', name=u'My Addôn', popularity=666)
        addon_factory(
            slug='my-theme', name=u'My Th€me', type=amo.ADDON_STATICTHEME
        )
        self.refresh()

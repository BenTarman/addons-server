...
class BaseTestEditDescribe(BaseTestEdit):
    ...
    def test_nav_links_uri_match(self):
        ...
        selected_link = (
            pq(response.content)('#edit-addon-nav')
            .find('li')
            .hasClass('selected')
        )
    ...
    def test_name_summary_lengths_long(self):
        self.client.post(
            self.describe_edit_url,
            self.get_dict(name='a' * 50, summary='b' * 50),
        )
    ...
    def test_name_summary_lengths_content_optimization(self):
        ...
        response = self.client.post(
            self.describe_edit_url,
            self.get_dict(name='a' * 2, summary='b' * 68),
        )
...
class TestEditDescribeListed(BaseTestEditDescribe, L10nTestsMixin):
    ...
    def test_edit_no_previous_categories(self):
        ...
        # has been updated.
        assert set(
            cat.id for cat in response.context['addon'].all_categories
...
class TestEditMedia(BaseTestEdit):
    ...
    def test_icon_dimensions_and_ratio(self):
        ...
        response = self.client.post(
            self.icon_upload,
            {'upload_image': open(get_image_path('icon64.png'), 'rb')},
        )
...
class TestEditDescribeStaticThemeUnlisted(
    StaticMixin, TestEditDescribeUnlisted
):

...
class TestAPIKeyPage(TestCase):
    ...
    def test_create_new_credentials_confirmation_exists_token_is_wrong(self):
        ...
        response = self.client.post(
            self.url,
            data={'action': 'generate', 'confirmation_token': 'wrong'},
        )
...
class TestLogout(UserViewBase):
    def test_success(self):
        ...
        assert pq(response.content)('li a.avatar').attr('href') == (
            user.get_url_path()
        )
        assert pq(response.content)('li a.avatar img').attr('src') == (
            user.picture_url
        )

...
def ownership(request, addon_id, addon):
    ...
    def mail_user_changes(
        author, title, template_part, recipients, extra_context=None
    ):
        ...
        for addon_user in addon_users_to_process:
            ...
            if not addon_user.pk:
                ...
                messages.success(
                    ...
                    ).format(email=addon_user.user.email),
                )
...
def json_file_validation(request, addon_id, addon, file_id):
    ...
    response = JsonResponse(
        {'validation': result.processed_validation, 'error': None,}
    )
    ...
    if 'upload_image' in request.FILES:
        ...
        if image_check.is_image() and content_waffle and is_preview:
            ...
            if actual_size[0] < min_size[0] or actual_size[1] < min_size[1]:
                ...
                errors.append(
                    ugettext(
                        'Image must be at least {0} pixels wide and {1} '
                        'pixels tall.'
...
def version_delete(request, addon_id, addon):
    ...
    if (
        ...
        and version.recommendation_approved
        and version == addon.current_version
    ):
        # Developers shouldn't be able to delete/disable the current version
        # of an approved add-on.
        msg = ugettext(
            'The latest approved version of a Recommended extension'
...
def _submit_details(request, addon, version):
    ...
    if show_all_fields:
        ...
        context.update(
            form=describe_form, cat_form=cat_form, policy_form=policy_form
        )
        forms_list.extend(
...
def api_key(request):
    ...
    if request.method == 'POST':
        has_confirmed_or_is_confirming = confirmation and (
            ...
            or confirmation.is_token_valid(
        ...
        if credentials and request.POST.get('action') in (
            'revoke',
            'generate',
        ):
        ...
        # the email.
            ...
            and request.POST.get('action') == 'generate'
        ):
...
def theme_background_image(request, addon_id, addon, channel):
    ...
    return (
        version.get_background_images_encoded(header_only=True)
        if version
        else {}
    )

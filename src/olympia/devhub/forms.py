...
class CategoryForm(forms.Form):
    ...
    def save(self, addon):
        ...
        categories_old = [
            c.id
            for c in addon.app_categories.get(
                amo.APP_IDS[application].short, []
            )
        ]
...
class BaseCompatFormSet(BaseModelFormSet):
    def __init__(self, *args, **kwargs):
        ...
        self.extra = (
            max(len(available_apps) - len(self.forms), 0)
            if not static_theme
            else 0
        )
...
class NewUploadForm(forms.Form):
    ...
    def __init__(self, *args, **kw):
        ...
        if self.addon and self.addon.current_version:
            ...
            compat_apps = list(
                self.addon.current_version.apps.values_list(
...
class SourceForm(WithSourceMixin, forms.ModelForm):
    ...
    def clean_source(self):
        ...
        if has_source == 'yes' and not source:
            ...
            raise forms.ValidationError(
                ugettext(
                    u'Source file uploaded but you indicated no source '
                    u'was needed.'
                )
            )
...
class DescribeForm(AddonFormBase):
    ...
    description = TransField(
        widget=TransTextarea(attrs={'rows': 6}), min_length=10
    )
...
class DescribeFormContentOptimization(
    CombinedNameSummaryCleanMixin, DescribeForm
):
...
class DescribeFormUnlisted(AddonFormBase):
    ...
    description = TransField(
        widget=TransTextarea(attrs={'rows': 4}), required=False
    )
...
class AgreementForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            ...
            and self.request.user.display_name
        ):
            ...
            self.fields[
                'display_name'
            ].validators += UserProfile._meta.get_field(
                'display_name'
            ).validators
...
class SingleCategoryForm(forms.Form):
    ...
    def __init__(self, *args, **kw):
        ...
        sorted_cats = sorted(
            CATEGORIES_NO_APP[self.addon.type].items(),
